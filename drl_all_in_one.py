...
class TabularQLearningAgent:
    """Q-learning agent for discrete route-planner action selection.

    The action space is the set of valid ``start_offset`` values for
    :class:`SimpleRoutePlanner`.  The state is a discretised bucket of
    the current best reward, giving the agent a lightweight signal about
    how well it is doing so far.

    This replaces the deterministic offset-cycling loop used previously,
    which produced a synthetic "convergence" rather than actual learning.
    With epsilon-greedy exploration (epsilon decaying from ``epsilon_start``
    to ``epsilon_end``) and Bellman Q-value updates the agent genuinely
    improves over training episodes instead of just interpolating rewards.
    """

    def __init__(
        self,
        num_actions: int,
        num_reward_buckets: int = 10,
        ...
        self.num_reward_buckets = num_reward_buckets
        ...
        # Q-table: state (reward bucket) → list of Q-values per action
        self.q_table: dict[int, list[float]] = {
            state: [0.0] * num_actions for state in range(num_reward_buckets)
        }

    def reward_to_state(self, reward: float, min_reward: float, max_reward: float) -> int:
        if max_reward <= min_reward:
            return 0
        normalized = (reward - min_reward) / (max_reward - min_reward)
        bucket = int(normalized * (self.num_reward_buckets - 1))
        return max(0, min(self.num_reward_buckets - 1, bucket))
...
def main() -> None:
    ...
    # Compute baseline using offset 0 for reward-bucket normalisation
    ...
    # Pre-evaluate all unique offsets so the agent can learn which are best
    route_cache: dict[int, tuple] = {}
    ...
    # TabularQLearningAgent learns which offset (action) yields the best routes
    ...
    current_state = agent.reward_to_state(best_reward, min_reward, max_reward)

    for episode in range(args.episodes):
        action = agent.select_action(current_state)
        candidate_routes, candidate_demand, candidate_travel, candidate_reward = route_cache[action]

        next_state = agent.reward_to_state(candidate_reward, min_reward, max_reward)
        agent.update(current_state, action, candidate_reward, next_state)
        agent.decay_epsilon()
        ...
        reward_tracker.record(candidate_reward)
        demands.append(candidate_demand)
        travel_times.append(candidate_travel)
